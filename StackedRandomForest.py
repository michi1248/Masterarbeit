...
class MultiDimensionalDecisionTree:
    def __init__(
        ...
        self.classification = (
            classification  # whether to use classification or regression
        )
    ...
    def build_training_data(
        ...
        for movement in tqdm.tqdm(
            ...
            emg_data, Mu_data, ref_data = open_all_files_for_one_patient_and_movement(
                path_to_subject_dat, movement
            )
            ref_erweitert = np.zeros(
                (self.num_movements, len(ref_data))
            ...
            self.labels_local_for_realtime.append(
                ref_erweitert
            )  # save data for later realtime processing
            emg_data = reshape_emg_channels_into_320(emg_data)
            emg_data = emg_data[self.important_channels, :]
            self.emg_data_local_for_realtime.append(
                emg_data
            )  # save data for later realtime processing
            for i in range(
                0, int(len(emg_data[0]) * stop), self.sample_difference_overlap
                ...
                for i in range(
                    ...
                    if (i <= ref_data.shape[0]) and (i - idx >= 0):
                        ...
                        difference_heatmap = normalize_2D_array(
                            np.subtract(heatmap, previous_heatmap)
                        )
                        ...
                        if self.classification:
                            ...
                            # best would be to take the ref from the signal because first comes the emg signal(heatmap) and the comes the reference or the real output
                            if (
                                i + self.neuromuscular_delay_in_samples
                            ) < ref_erweitert[0].shape[0]:
                                for skip in range(
                                    64, self.neuromuscular_delay_in_samples, 64
                                ):
                                    ref_in_the_future = ref_erweitert[:, i + skip]
    ...
    def load_trainings_data(self):
        self.X_test_local = np.array(
            load_pickle_file(
                r"D:\Lab\MasterArbeit\trainings_data\X_test_local_"
                + str(self.window_size)
                + "_"
                + str(self.sample_difference_overlap)
                + ".pkl"
            )
        )
        self.y_test_local = np.array(
            load_pickle_file(
                r"D:\Lab\MasterArbeit\trainings_data\y_test_local_"
                + str(self.window_size)
                + "_"
                + str(self.sample_difference_overlap)
                + ".pkl"
        ...
        self.X_train_local = np.array(
            load_pickle_file(
                r"D:\Lab\MasterArbeit\trainings_data\X_train_local_"
                + str(self.window_size)
                + "_"
                + str(self.sample_difference_overlap)
                + ".pkl"
        ...
        self.y_train_local = np.array(
            load_pickle_file(
                r"D:\Lab\MasterArbeit\trainings_data\y_train_local_"
                + str(self.window_size)
                + "_"
                + str(self.sample_difference_overlap)
                + ".pkl"
        ...
        self.training_data_time = load_pickle_file(
            r"D:\Lab\MasterArbeit\trainings_data\training_data_time_"
            + str(self.window_size)
            + "_"
            + str(self.sample_difference_overlap)
            + ".pkl"
        ...
        for i in tqdm.tqdm(range(len(self.trees)), desc="Evaluating trees"):
            if i == 0:
                ...
                if not self.classification:
                    ...
                    print(
                        "local precision: ",
                        precision_score(truth, res, average="weighted"),
                    )
                    print(
                        "local recall: ", recall_score(truth, res, average="weighted")
                    )
                ...
                if not self.classification:
                    ...
                    print(
                        "time accuracy for tree " + str(i) + " : ",
                        accuracy_score(truth, res),
                    )
                    print(
                        "time precision for tree " + str(i) + " : ",
                        precision_score(truth, res, average="weighted"),
                    )
                    print(
                        "time recall for tree " + str(i) + " : ",
                        recall_score(truth, res, average="weighted"),
                    )
        if give_best_time_tree:
        ...
        for i in tqdm.tqdm(range(len(self.trees) - 1), desc="Evaluating trees"):
            ...
            if not self.classification:
                ...
                mae = mean_absolute_error(
                    truth[difference:], res, multioutput="raw_values"
                )
                ...
                self.compare_predictions(res, truth[difference:], tree_number=i)
                print(
                    "combined accuracy for tree " + str(i) + " : ",
                    accuracy_score(truth[difference:], res),
                )
                print(
                    "combined precision for tree " + str(i) + " : ",
                    precision_score(truth[difference:], res, average="weighted"),
                )
                print(
                    "combined recall for tree " + str(i) + " : ",
                    recall_score(truth[difference:], res, average="weighted"),
                )
    ...
    def realtime_prediction(self):
        ...
        self.build_training_data(
            self.movements,
            r"D:\Lab\data\extracted\Sub2",
            split_ratio=split_number,
        ...
        emg_datas = (
            self.emg_data_local_for_realtime
        )  # [num_movements x #channels x #samples]
        ...
        for movement in range(len(emg_datas)):
            ...
            results_local = []
            for i in range(
                int(len(emg_data[movement]) * split_number),
                int(len(emg_data[movement])) - 65,
                64,
            ):  # gehe über emg in schritten von 64 da 64 samples in chunks von emg bekomme in echt
                ...
                if i <= labels[movement].shape[1]:
                    ...
                    heatmap_local = calculate_emg_rms_row(
                        data, data[0].shape[0], self.window_size_in_samples
                    )
                    ...
                    for past_times in self.num_previous_samples:
                        previous_heatmap = calculate_emg_rms_row(
                            data,
                            data[0].shape[0] - past_times,
                            self.window_size_in_samples,
                        )  # TODO maybe change to less samples like 64
                        previous_heatmap = normalize_2D_array(previous_heatmap)
                        difference_heatmap = normalize_2D_array(
                            np.subtract(heatmap_local, previous_heatmap)
                        )
            ...
            self.compare_predictions(
                np.array(results_local),
                np.array(truths),
                tree_number="local",
                realtime=True,
            )
            list_with_time_predictions_for_all_times_and_movements.append(
                predictions_time
            )
        ...
        for movement in range(len(emg_datas)):
            for i in range(len(self.num_previous_samples)):
                self.compare_predictions(
                    ...
                    np.array(all_truths[movement]),
                    tree_number=str("time : " + str(i)),
                    realtime=True,
                )
        ...
        self.realtime_evaluation(
            np.array(all_truths),
            np.array(all_local_predictions),
            np.array(all_time_predictions),
        )
