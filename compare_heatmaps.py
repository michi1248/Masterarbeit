...
class Heatmap:
    ...
    def make_heatmap_emg(self,frame):
        ...
        if self.global_counter == 0:
            hmap = sns.heatmap(normalized_heatmap, ax=self.ax_emg, cmap='RdBu_r', cbar=True, xticklabels=True, yticklabels=True, cbar_kws={'label': 'norm. RMS'},mask=self.mask_for_heatmap)#,cbar_ax=self.ax_emg,)
        else:
            hmap = sns.heatmap(normalized_heatmap, ax=self.ax_emg, cmap='RdBu_r', cbar=False, xticklabels=True, yticklabels=True, cbar_kws={'label': 'norm. RMS'},mask=self.mask_for_heatmap)#,cbar_ax=self.ax_emg,)
    ...
    def make_mu_heatmap(self,frame):
        ...
        normalized_heatmap = normalize_2D_array(heatmap)
        sns.heatmap(normalized_heatmap, ax=self.ax_MU, cmap='RdBu_r', cbar=False, xticklabels=True, yticklabels=True, cbar_kws={'label': 'norm. RMS'})
    ...
    def mu_with_closest_spike(self,frame):
        ...
        for mu in range(self.Mu_data.shape[1]):
            closest_value = min(self.Mu_data[0][mu][0][:], key=lambda value: abs(value - frame))
    ...
    def mu_with_most_spikes_since_last_frame(self,frame):
        ...
        for mu in range(self.Mu_data.shape[1]):
            number_firings = np.count_nonzero((self.Mu_data[0][mu][0][:] > self.last_frame) & (self.Mu_data[0][mu][0][:] < frame))
...
if __name__ == "__main__":
    ...
    fps = 2
    heatmap.save_animation(r"D:\Lab\MasterArbeit\plots\Heatmaps\emg_animation_" +str(movement) +str(fps) +"fps_most_firings.gif", fps=2)
