...
def extract_mat_to_pkl(
    path_to_save,
    emg_folder,
    MU_folder,
    movement_ground_truth_folder,
    attributes_to_open=None,
):
    movement_list = [
        "thumb_slow",
        "thumb_fast",
    ...
    for subject in os.listdir(emg_folder):
        ...
        if (not "DATATABLES" in subject) and (not "Ordner_Struktur" in subject):
            ...
            for movement in tqdm.tqdm(
                ...
                desc="extracting emg data for " + subject + "...",
            ):
                ...
                movement_name = movement_list[
                    int(movement.split("_")[1].split(".")[0]) - 1
                ]

                # load mat file
                mat_data = sio.loadmat(movement_file_path)
                # save data in dict
                data_dict[movement_name] = {
                    key: mat_data[key] for key in attributes_to_open
                }

            # extract ref data
            subject_name = subject.split("Sub")[1]
            ref_file_path = os.path.join(
                movement_ground_truth_folder, "Exp" + subject_name + "_B_DATATABLE.mat"
            )
...
if __name__ == "__main__":
    ...
    emg_data, MU_data, ref_data = open_all_files_for_one_patient_and_movement(
        folder_path=r"D:\Lab\data\extracted\sub2_Charly", movement_name="index_slow"
    )
