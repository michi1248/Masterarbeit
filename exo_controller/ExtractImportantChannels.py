...
class ChannelExtraction:
    def __init__(self, movement_name, emg,ref, sampling_frequency=2048, frame_duration=150):
        """
    ...
    def make_heatmap_emg(self, frame):
        ...
        if (is_near_extremum(frame, self.local_maxima, self.local_minima, time_window=self.frame_duration,
                                     sampling_frequency=self.sampling_frequency)):
            # find out to which part of the movement the current sample belongs (half of flex or ref)
            belongs_to_movement, distance = check_to_which_movement_cycle_sample_belongs(frame, self.local_maxima,
                                                                                                 self.local_minima)
...
if __name__ == "__main__":
    movement_list = ["thumb_slow", "thumb_fast", "index_slow", "index_fast",]
    important_channels = []
