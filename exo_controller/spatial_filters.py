...
class Filters:
    ...
    def spatial_filtering(self, siganl_to_be_filtered, filter_name= "NDD",mode="valid"):
        ...
        for one_grid in range(len(grids)):
            if one_grid < 3:  # if upper grids
                grid[0:8, 0 + 8 * one_grid:8 + 8 * one_grid,:] = fftconvolve(grids[one_grid], self._DIFFERENTIAL_FILTERS[filter_name].reshape((self._DIFFERENTIAL_FILTERS[filter_name].shape[0],self._DIFFERENTIAL_FILTERS[filter_name].shape[1],1)), mode=mode).astype(np.float32)
            else:  # if lower grids
                grid[8:16, 0 + 8 * (one_grid - 3):8 + 8 * (one_grid - 3),:] = fftconvolve(grids[one_grid], self._DIFFERENTIAL_FILTERS[filter_name].reshape((self._DIFFERENTIAL_FILTERS[filter_name].shape[0],self._DIFFERENTIAL_FILTERS[filter_name].shape[1],1)), mode=mode).astype(np.float32)
    ...
    def bandpass_filter_emg_data(self,emg_data, fs = 2048):
        ...
        for channel in range(emg_data.shape[0]):
            data[channel] = self.bandpass_filter(emg_data[channel], 10, 500, fs, order=5)
        return data
    ...
    def apply_gaussian_filter(self,grid, gaussian_filter):
        ...
        for one_grid in range(len(grids)):
            if one_grid < 3:  # if upper grids
                ...
                grid[8:16, 0 + 8 * (one_grid - 3):8 + 8 * (one_grid - 3)] = filter_function(grids[one_grid],
                                                                                            gaussian_filter)
