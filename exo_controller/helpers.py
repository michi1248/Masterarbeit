...
def extract_mean_heatmaps(movement_list, path_to_subject_data, emg=None, ref=None):
    ...
    for movement in tqdm.tqdm(
        movement_list, desc="Extracting mean heatmaps for from all movements"
    ):
...
def calculate_emg_rms_row(emg_grid, position, interval_in_samples):
    ...
    for row_idx in range(num_rows):
        if (position - interval_in_samples < 0) or (
            emg_grid[row_idx].shape[0] < (interval_in_samples)
        ):
...
def resample_reference_data(ref_data, emg_data):
    ...
    for movement in ref_data.keys():
        ref_data[movement] = resample(
            ref_data[movement], emg_data[movement].shape[1], axis=0
        )
    return ref_data
...
def calculate_emg_rms_grid(emg_grid, position, interval_in_samples):
    ...
    for row_idx in range(num_rows):
        for col_idx in range(num_cols):
            if (position - interval_in_samples < 0) or (
                len(emg_grid[row_idx][col_idx][0]) < (interval_in_samples)
            ):
                channel_data = emg_grid[row_idx][col_idx][0][: position + 1]
            else:
                channel_data = emg_grid[row_idx][col_idx][0][
...
def from_grid_position_to_row_position(grid_position):
    ...
    if grid_position[0] < 8:
        ...
        row_position = (
            adding + grid_position[0] * 8 + ((grid_position[1] - 1) - (grid - 1) * 8)
        )
    else:
        ...
        row_position = (
            adding + (grid_position[1] * 5 - ((grid - 1) * 13)) + ((grid_position[0]))
        )

    if grid_position in outliers:
        print(
            "error in helper function from_grid_position_to_row_position , this positon is an outlier"
        )
    else:
        if row_position > 320:
            print(
                "error in helper function from_grid_position_to_row_position , this positon is out of range"
            )
    return row_position


def find_max_min_values_for_each_movement_and_channel(
    emg_data, important_channels, movements
):
    """
...
def find_q_median_values_for_each_movement_and_channel(
    emg_data, important_channels, movements
):
    """
    This function finds the max and min values for each channel in the emg data.
...
def spikeTriggeredAveraging(SIG, MUPulses, STA_window, emg_length, fsamp):
    ...
    STA_mean = np.zeros(
        (MUPulses.shape[1], SIG.shape[0], SIG.shape[1], 2 * STA_window)
    )  # Initialize STA_mean with #of MUs, #rows, and #columns
        ...
        for row in range(SIG.shape[0]):  # ... extract for each row ...
            ...
            for col in range(SIG.shape[1]):  # ... and column of the grid ...
                if len(SIG[row][col][0]) > 1:  # Check if the channel is not empty
                    ...
                    for spks in MUPulses[0, MU][
                        0
                    ]:  # ... the raw EMG signal of that channel inside the STA window around each MU firing.
                        if (spks + STA_window < emg_length) and (
                            spks - STA_window >= 0
                        ):  # check if the STA window is within the emg signal
                            temp_STA.append(
                                SIG[row][col][0][spks - STA_window : spks + STA_window]
                            )  # fill temp STA with the emg signal around the MU firing for all spikes

                    STA_mean[MU][row][col][:] = np.nanmean(
                        np.array(temp_STA).reshape(len(temp_STA), len(temp_STA[0])),
                        axis=0,
                    )  # Compute mean of all these extracted EMG signals
    return STA_mean


def check_to_which_movement_cycle_sample_belongs(
    sample_position, local_maxima, local_minima, method="closest_extrema"
):
    """
    ...
    if method == "marius":
        ...
        if abs(sample_position - closest_maxima) < abs(
            sample_position - closest_minima
        ):
            return 1, abs(sample_position - closest_maxima)
        else:
            return 2, abs(sample_position - closest_minima)
...
def plot_predictions(ground_truth, prediction, tree_number=None, realtime=False):
    ...
    if ground_truth.ndim == 1:
        ...
        plt.scatter(
            x_values,
            prediction[:, 0],
            label="flexion value prediction",
            color="green",
            s=10,
        )
        ...
        plt.legend()
        plt.title(
            "Ground Truth 1 vs. Predictions for tree number : " + str(tree_number)
        )
        plt.xlabel("Sample Index")
        plt.ylim(0, 1)
        plt.ylabel("Values")
        ...
        plt.scatter(
            x_values,
            prediction[:, 1],
            label="extension value prediction",
            color="green",
            s=10,
        )
        ...
        plt.legend()
        plt.title(
            "Ground Truth 2 vs. Predictions for tree number : " + str(tree_number)
        )
        plt.xlabel("Sample Index")
        plt.ylabel("Values")
...
def plot_local_maxima_minima(
    ...
    if current_sample_position is not None:
        plt.plot(
            ...
            movement_signal[current_sample_position],
            "x",
            color=color,
        )
    plt.show()
        ...
        for j in range(difference_heatmap.shape[1]):
            ...
            for x_offset in [-1, 0, 1]:
                for y_offset in [-1, 0, 1]:
                    if (
                        0 <= i + x_offset < difference_heatmap.shape[0]
                        and 0 <= j + y_offset < difference_heatmap.shape[1]
                    ):
                        if x_offset == 0 and y_offset == 0:
                            continue
                        neighbours.append(
                            difference_heatmap[i + x_offset, j + y_offset]
                        )
                        differences.append(
                            np.abs(
                                difference_heatmap[i + x_offset, j + y_offset]
                                - difference_heatmap[i, j]
                            )
                        )
            ...
            if (
                (min(differences) <= threshold_neighbours)
                and (this_value > max_neighbour)
                and (this_value > threshold_difference_amplitude)
            ):
            ...
            if (
                this_value <= min(neighbours)
                and (min(differences) <= threshold_neighbours)
                and (this_value is not None)
                and (this_value != 0.0)
            ):
                activity_in_movement_1 = mean_flex_heatmap[i][j]
                activity_in_movement_2 = mean_ex_heatmap[i][j]
...
def is_near_extremum(
    frame, local_maxima, local_minima, time_window, sampling_frequency
):
    """
    ...
    if (
        distance_to_maxima < time_window_in_samples
        or distance_to_minima < time_window_in_samples
    ):
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    for i in range(num_periods):
        ...
        signal[start_idx:end_idx] = amplitudes[i] * np.sin(
            2 * np.pi * frequency * t[start_idx:end_idx]
        )

    sample_position = 49  # Replace with the desired sample position

    maxima, minima = get_locations_of_all_maxima(signal)
    result = check_to_which_movement_cycle_sample_belongs(
        sample_position, maxima, minima
    )
    plt.figure()
